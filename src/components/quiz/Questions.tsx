  ...
  if (!questions || questions.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-green-700">Пока нет вопросов. Вернитесь позже.</p>
      </div>
    )
  }
        ...
        >
          ...
          <div className="flex items-start gap-4 mb-4">
            <span className={`text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-semibold flex-shrink-0 ${
              hasError ? 'bg-red-600' : 'bg-green-700'
            }`}>
              {index + 1}
            </span>
            <h3 className="text-lg font-semibold text-green-900 pt-1">
              ...
              {question.options.map((option) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm ${selected ? 'text-green-900 font-medium' : 'text-green-800'}`}>
                      {option}
                    </span>
                  </button>
                )
          ...
          {question.question_type === 'multiple' && (
            <div className="space-y-3 pl-12">
              {question.options.map((option) => (
                <div key={option} className="flex items-center space-x-2 cursor-pointer hover:bg-green-50 rounded-md p-2 -ml-2 transition-colors">
                  <Checkbox
                  ...
                  <label
                    ...
                    className="text-green-800 cursor-pointer flex-1"
                  >
                    {option}
                  </label>
                </div>
          ...
          {question.question_type === 'text' && (
            <div className="pl-12">
              <Input
                ...
                className="max-w-md bg-white border-green-300 text-green-900 placeholder-green-400"
              />
            </div>
          )}
            ...
            <div className="pl-12">
              <div className="flex gap-2 max-w-md">
                <Input
                  ...
                  className="flex-1 bg-white border-green-300 text-green-900 placeholder-green-400"
                />
                {question.units && question.units.length > 0 && (
                  <div className="relative" ref={unitDropdownOpen === question.id ? unitDropdownRef : undefined}>
                    <button
                      ...
                      className="flex items-center justify-between gap-3 border border-green-300 rounded-md px-3 py-2 text-green-800 bg-white hover:border-green-500 focus:outline-none focus:border-green-500 transition-colors min-w-[120px] cursor-pointer"
                    >
                      ...
                      <ChevronDown
                        size={16}
                        className={`text-green-600 flex-shrink-0 transition-transform ${unitDropdownOpen === question.id ? 'rotate-180' : ''}`}
                      />
                    </button>
                    {unitDropdownOpen === question.id && (
                      <div className="absolute top-full mt-1 left-0 z-20 bg-white border border-green-200 rounded-lg shadow-lg min-w-full overflow-hidden">
                        {question.units.map((unit) => {
                          ...
                          return (
                            <button
                              ...
                              className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-sm text-left hover:bg-green-50 transition-colors cursor-pointer ${
                                isSelected ? 'text-green-900 font-medium bg-green-50' : 'text-green-800'
                              }`}
                            >
                              <span>{unit}</span>
            ...
            <div className="pl-12">
              ...
              >
                <DialogTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-full max-w-md justify-start text-left cursor-pointer bg-white border-green-300 text-green-800 hover:border-green-500"
                  >
                ...
                <DialogContent className="sm:max-w-sm bg-white border border-green-200">
                  <DialogHeader>
                    <DialogTitle className="text-green-900">Выберите дату</DialogTitle>
                  </DialogHeader>
                  <div className="py-2">
                    <CalendarComponent
                      mode="single"
                      ...
                      className="rounded-md border border-green-200 w-full"
                    />
                  </div>
                  <div className="flex justify-end gap-2 pt-2">
                    <DialogClose asChild>
                      <Button variant="outline" className="cursor-pointer bg-white border-green-300 text-green-800 hover:bg-green-50">Отмена</Button>
                    </DialogClose>
                    <DialogClose asChild>
                      <Button
          ...
          {question.question_type === 'time' && (
            <div className="pl-12">
              <Button
                ...
                className="w-full max-w-md justify-start bg-white border-green-300 text-green-800 hover:border-green-500"
              >
