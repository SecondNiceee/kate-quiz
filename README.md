...
Проект поднимается в 4 шага. **Порядок важен:** сначала `pnpm i`, только потом `pnpm run setup`.
...
Обязательно делайте это **первым шагом**. Скрипт заливки базы (`pnpm run setup`) использует пакеты `pg` и `dotenv` из `node_modules` — без установленных зависимостей он просто не запустится.
...
```bash
pnpm i                    # 1. зависимости — обязательно первыми
cp .env.example .env      # 2. затем заполнить переменные
pnpm run setup            # 3. залить start.sql в базу
pnpm build && pnpm start  # 4. собрать и запустить
```
